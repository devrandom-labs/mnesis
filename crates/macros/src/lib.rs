...
fn parse_domain_event(ast: &DeriveInput) -> Result<proc_macro2::TokenStream> {
    ...
    match &ast.data {
        Data::Enum(data_enum) => {
            ...
            let expanded = quote! {
                impl ::mnesis::Message for #name {}

                impl ::mnesis::DomainEvent for #name {
...
fn parse_transforms(
    ...
    let match_arms: Vec<_> = transforms
        ...
        .map(|t| {
            ...
            quote! {
                (#event_type, v) if v == ::mnesis::Version::new(#from_version).expect("nonzero") => {
                    let payload = Self::#fn_name(morsel.payload())?;
                    ::mnesis_store::upcasting::EventMorsel::new(
                        #output_event_type,
                        ::mnesis::Version::new(#to_version).expect("nonzero"),
                        payload,
                    )
                }
            }
        ...
        .map(|(event_type, version)| {
            quote! {
                #event_type => ::core::option::Option::Some(
                    ::mnesis::Version::new(#version).expect("nonzero")
                )
            }
        })
        .collect();
    ...
    let expanded = quote! {
        ...
        impl #struct_ident {
            ...
            /// current schema version. Generated by `#[mnesis::transforms]`.
            pub fn upcast<'a>(
                mut morsel: ::mnesis_store::upcasting::EventMorsel<'a>,
            ) -> ::core::result::Result<
                ::mnesis_store::upcasting::EventMorsel<'a>,
                #error_type,
            > {
                loop {
            ...
            /// Generated by `#[mnesis::transforms]`.
            #[must_use]
            pub fn current_version(event_type: &str) -> ::core::option::Option<::mnesis::Version> {
                match event_type {
                    #(#version_arms,)*
