//! Tests for `mnesis::testing::SagaFixture`. Exercises the real
...
use mnesis::{AggregateState, DomainEvent, Events, Message, React, Saga, events};
...
#[mnesis::aggregate(state = OrderSagaState, error = OrderSagaError, id = OrderId)]
struct OrderSaga;
...
mod small_limit {
    ...
    use mnesis::testing::SagaFixture;
    use mnesis::{Aggregate, AggregateState, DomainEvent, Message, Saga};
