...
The 56 proptest tests above are just one tool. Below is the complete landscape of techniques that apply to mnesis-store, ordered by implementation priority.
...
```bash
# Full crate
cargo mutants -p mnesis-store

# Just the core modules
cargo mutants -p mnesis-store -- --file src/upcaster.rs --file src/event_store.rs
...
```bash
# Run all mnesis-store tests under Miri
cargo +nightly miri test -p mnesis-store

# Run specific tests (proptest is slow under Miri — use unit tests)
cargo +nightly miri test -p mnesis-store --test envelope_tests
cargo +nightly miri test -p mnesis-store --test error_tests

# With extra flags
MIRIFLAGS="-Zmiri-many-seeds=64" cargo +nightly miri test -p mnesis-store
```
...
miri:
  ...
  steps:
    ...
    - run: cargo +nightly miri test -p mnesis -p mnesis-store --test envelope_tests --test error_tests --test stream_tests --test upcaster_tests
```
...
cargo kani --tests -p mnesis-store
```
...
cargo bolero test -p mnesis-store fuzz_codec_decode_never_panics
# libfuzzer backend (deep, needs nightly + Linux)
cargo +nightly bolero test -p mnesis-store fuzz_codec_decode_never_panics --engine libfuzzer
```
...
```bash
cargo +nightly careful test -p mnesis-store
```
...
**When:** Only if mnesis ever supports multi-node event stores.

**Where:** A separate `mnesis-distributed-tests` crate that models the store as an actor system.
...
cargo test -p mnesis-store --test nasa_break_everything

# Miri (slow, catches UB)
cargo +nightly miri test -p mnesis -p mnesis-store --test envelope_tests --test error_tests
...
pr-checks:
  - cargo test -p mnesis-store --test nasa_break_everything
  - cargo test -p mnesis-store  # all other tests

# Every PR — UB detection
miri:
  - cargo +nightly miri test -p mnesis -p mnesis-store --test envelope_tests --test error_tests --test upcaster_tests
...
nightly:
  - PROPTEST_CASES=2048 cargo test -p mnesis-store --test nasa_break_everything
  # + per-adapter conformance when adapters exist

# Per-adapter (when fjall/sqlite land)
adapter:
  - cargo test -p mnesis-fjall --test conformance_tests
  - PROPTEST_CASES=2048 cargo test -p mnesis-fjall --test conformance_tests
```
