...
`nix develop -c cargo nextest run -p mnesis-store-testing -p mnesis-inmemory -p mnesis-fjall` — everything green (a failure of a NEW check against any adapter is a real finding → BLOCKED, report, never weaken). fmt, `git add`, commit:
...
- `git rm execute_lifecycle_tests.rs` (facade-through-fjall reopen; kit lifecycle + mnesis-store facade tests cover both halves — READ the file first and confirm it holds nothing else; if it tests facade behavior not covered anywhere, reclassify to C and report)
...
- [x] **Step 2.8:** prune unused imports/helpers orphaned by the deletions in each touched file; `nix develop -c cargo nextest run -p mnesis-fjall` green; fmt; commit `refactor(fjall): drop tests the conformance kit now owns (#281)`.
...
- [x] **Step 3.3:** prune imports; `nix develop -c cargo nextest run -p mnesis-inmemory` green; fmt; commit `refactor(inmemory): drop tests the conformance kit now owns (#281)`.
...
- [x] **Step 4.4:** `nix develop -c cargo nextest run -p mnesis-postgres` green (vacuous conformance skips + surviving tests); fmt; commit `refactor(postgres): drop tests the conformance kit now owns (#281)`.
...
| 5 | final review | added `check_subscription_beyond_head_filters_below_bound` to the kit (sequence + `conformance!`) | whole-branch review found deleting fjall's `subscribe_from_beyond_head` left the below-bound-filter observable asserted nowhere: live appends at versions <= `from` must wake but never be delivered, first delivery at `from+1` | hole closed on all three adapters, strictly stronger than the deleted fjall-local test |
