...
- 2026-07-16, Task 4 review (`9c390d0`): lifecycle test reworked to exercise a REAL resume — reopen now restarts the loop from the recovered `(seed, checkpoint)`, appends a second todo, and asserts the exact two-todo page (the non-idempotent `Created` fold makes a re-delivered event 1 fail deterministically). Plan's version only asserted hydrate returned the pair.
- 2026-07-16, Task 5 review (`d07175c`): two additions the plan never specified — an `internal()` helper so no handler error is silently collapsed to 500 (rule 3: every error path here is new, non-upstream surface), and a `503 Service Unavailable` liveness guard on `GET /todos` (dead projection loop = dropped watch sender; without it frozen reads serve as 200s forever). 503 joins 409 as a sanctioned contract divergence — reflected in README/lib.rs/PROVENANCE and issue #332.
