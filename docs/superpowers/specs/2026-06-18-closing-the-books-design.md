...
**Issue:** devrandom-labs/mnesis#139 (Tier 1: Table Stakes — last open card)
...
1. A doc-only rustdoc module in the `mnesis` kernel crate (`closing_the_books`) — the guide,
...
**New file:** `crates/mnesis/src/closing_the_books.rs` — a doc-only module: a `//!` guide with
**no code items**. Declared in `crates/mnesis/src/lib.rs` as `pub mod closing_the_books;`.

- **Not feature-gated.** Docs must never disappear behind a flag. Zero code, zero deps, always
  compiled. (The `mnesis` crate opts into god-mode clippy via `[lints]`, but a doc-only module has
...
**Cross-link direction (load-bearing constraint):** the `mnesis` kernel does **not** depend on
`mnesis-store`, so the guide can *name* the `Snapshotting` decorator in prose but cannot make a
clickable intra-doc link up to it. The link goes the other way: add a one-line pointer in
`mnesis-store`'s `snapshot.rs` module docs — "before reaching for this, consider
[`mnesis::closing_the_books`]" — which **is** a resolvable intra-doc link because `mnesis-store`
depends on `mnesis`. This puts discoverability at the snapshot API, where the user needs the advice.

## Deliverable 2 — runnable example `examples/closing-the-books/`

A **pure-kernel** example (depends on `mnesis` with `derive` + `testing`, plus `thiserror` and
...
Use `mnesis::testing::AggregateFixture<CashierShift>` to assert the only non-trivial decision
...
| File | Change |
|------|--------|
| `crates/mnesis/src/closing_the_books.rs` | **new** — doc-only `//!` guide module |
| `crates/mnesis/src/lib.rs` | add `pub mod closing_the_books;` |
| `crates/mnesis-store/src/snapshot.rs` | add intra-doc pointer to `mnesis::closing_the_books` in the module/decorator docs |
...
- No new kernel/store API, no "close stream"/"archive" primitive. Closing the Books in mnesis is a
...
- **Cross-crate intra-doc link** — the pointer must live in `mnesis-store` (→ `mnesis`), never in
  `mnesis` (→ `mnesis-store`), which would not resolve. Captured above.
