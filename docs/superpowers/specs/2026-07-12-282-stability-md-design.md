...
| On-disk format | **Major-bounded + one-major read overlap**: within 1.x the default write format stays frame v1 and every 1.x reads everything any 1.x wrote; 2.x may write a new format but must still read 1.x's. Beyond one major: export → CBOR box → import. Unknown format bytes fail typed (`UnsupportedFrameVersion`), never misparse. Same policy for the CBOR box `format_version`. |
...
2. **Semver surface** —
   - **In**: every documented `pub` item of 1.0-tier crates; *documented trait semantics* (inclusive read bounds, strict-after resume, `CaughtUp` exactly once, conflict-rejects-atomically — behavior, not just signatures); documented `Send`/`Sync` bounds; the two acknowledged public dependencies `bytes` and `futures-core` (a major bump of either forces our major; everything else was sealed by #208).
