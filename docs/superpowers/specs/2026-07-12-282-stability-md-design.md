...
**Issue:** [#282](https://github.com/joeldsouzax/mnesis/issues/282) · **Milestone:** 1 — Pre-Freeze (1.0 blockers)
...
| Fork | Decision |
|------|----------|
| Crate tiers | **Core + wake at 1.0**: `mnesis`, `mnesis-macros`, `mnesis-store`, `mnesis-wake`, `mnesis-wake-nostd`. **0.x**: `mnesis-inmemory`, `mnesis-fjall`, `mnesis-postgres`, `mnesis-store-testing`. Unpublished: `workspace-hack`, `nostd-smoketest`, examples. |
...
1. **Crate tiers** — the table above, plus coupling rules:
   - `mnesis-macros` is version-locked to `mnesis` (serde/serde_derive precedent).
   - A `mnesis` major forces a `mnesis-store` major (kernel types are re-exported in store APIs).
   - A `mnesis-store` major forces majors of both wake crates (they implement its public traits in their public API).
...
- **`mnesis-wake` finding**: it still carries both the watch-generation path and the legacy `Notify`/`wake_all` transition path. Promoting it to 1.0 freezes that public surface — resolve as either a pre-freeze cleanup card or an explicit "intentional, frozen" note in the doc.
...
- No behavior or API changes — this PR is documentation plus issue comments; the only code-adjacent outcome is possibly filing a new cleanup card for `mnesis-wake`.
